#!/usr/bin/env bash
# Is this checkout exactly the release that the version in src/reckon_db.app.src
...
if [ "${#REASONS[@]}" -gt 0 ]; then
    echo "REFUSED to publish reckon_db $VSN from $REPO_DIR:"
    printf '  - %s\n' "${REASONS[@]}"
    exit 1
fi
