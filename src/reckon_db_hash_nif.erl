...
%% Partition = reckon_db_hash_nif:partition_hash(StreamId, 16).
%%
%% %% Stream-specific routing
%% Partition = reckon_db_hash_nif:stream_partition(StoreId, StreamId, 16).
%%
%% %% Raw xxHash for checksums
%% Hash = reckon_db_hash_nif:xxhash64(Data).
%%
%% %% Check which mode is active
%% nif = reckon_db_hash_nif:implementation().  %% Enterprise
%% erlang = reckon_db_hash_nif:implementation(). %% Community
%% '''
%%
%% @author rgfaber

-module(reckon_db_hash_nif).
...
init() ->
    ...
    case try_load_nif(Paths) of
        ...
        {error, Reason} ->
            ...
            logger:info("[reckon_db_hash_nif] NIF not available (~p), using pure Erlang - Community mode",
                       [Reason]),
            ok
    end.
