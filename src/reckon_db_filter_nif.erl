...
%% true = reckon_db_filter_nif:wildcard_match(<<"orders-123">>, <<"orders-*">>).
%%
%% %% Filter streams by pattern
%% Matching = reckon_db_filter_nif:filter_by_wildcard(Streams, <<"user-*">>).
%%
%% %% Check which mode is active
%% nif = reckon_db_filter_nif:implementation().  %% Enterprise
%% erlang = reckon_db_filter_nif:implementation(). %% Community
%% '''
%%
%% @author rgfaber

-module(reckon_db_filter_nif).
...
-define(NIF_LOADED_KEY, reckon_db_filter_nif_loaded).
...
-spec init() -> ok.
init() ->
    NifName = "reckon_db_filter_nif",
    Paths = nif_search_paths(NifName),
    case try_load_nif(Paths) of
        ok ->
            persistent_term:put(?NIF_LOADED_KEY, true),
            logger:info("[reckon_db_filter_nif] NIF loaded - Enterprise mode"),
            ok;
        {error, Reason} ->
            persistent_term:put(?NIF_LOADED_KEY, false),
            logger:info("[reckon_db_filter_nif] NIF not available (~p), using pure Erlang - Community mode",
                       [Reason]),
            ok
    end.
