...
%% true = reckon_db_crypto_nif:verify_ed25519(Message, Signature, PublicKey).
%%
%% %% Generate token CID (SHA256 + base64)
%% CID = reckon_db_crypto_nif:hash_sha256_base64(Data).
%%
%% %% Check which mode is active
%% true = reckon_db_crypto_nif:is_nif_loaded().  %% Enterprise
%% false = reckon_db_crypto_nif:is_nif_loaded(). %% Community
%% '''
%%
%% @author rgfaber

-module(reckon_db_crypto_nif).
...
-define(NIF_LOADED_KEY, reckon_db_crypto_nif_loaded).
...
-spec init() -> ok.
init() ->
    NifName = "reckon_db_crypto_nif",
    Paths = nif_search_paths(NifName),
    case try_load_nif(Paths) of
        ok ->
            persistent_term:put(?NIF_LOADED_KEY, true),
            logger:info("[reckon_db_crypto_nif] NIF loaded - Enterprise mode"),
            ok;
        {error, Reason} ->
            persistent_term:put(?NIF_LOADED_KEY, false),
            logger:info("[reckon_db_crypto_nif] NIF not available (~p), using pure Erlang - Community mode",
                       [Reason]),
            ok
    end.
