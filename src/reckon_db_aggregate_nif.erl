...
%% Total = reckon_db_aggregate_nif:sum_field(Events, amount).
%%
%% %% Check which mode is active
%% nif = reckon_db_aggregate_nif:implementation().  %% Enterprise
%% erlang = reckon_db_aggregate_nif:implementation(). %% Community
%% '''
%%
%% @author rgfaber

-module(reckon_db_aggregate_nif).
...
-define(NIF_LOADED_KEY, reckon_db_aggregate_nif_loaded).
...
-spec init() -> ok.
init() ->
    NifName = "reckon_db_aggregate_nif",
    Paths = nif_search_paths(NifName),
    case try_load_nif(Paths) of
        ok ->
            persistent_term:put(?NIF_LOADED_KEY, true),
            logger:info("[reckon_db_aggregate_nif] NIF loaded - Enterprise mode"),
            ok;
        {error, Reason} ->
            persistent_term:put(?NIF_LOADED_KEY, false),
            logger:info("[reckon_db_aggregate_nif] NIF not available (~p), using pure Erlang - Community mode",
                       [Reason]),
            ok
    end.
