...
%%
%% @author rgfaber
%% @see reckon_gater_capability
%% @see reckon_gater_identity
...
verify(Token, Opts) ->
    case reckon_gater_capability:decode(Token) of
        {ok, Cap} ->
...
is_revoked(_TokenCID) ->
    ...
    %% reckon_db_revocation:is_revoked(TokenCID)
    false.
...
%% Must match the format used in reckon_gater_capability:sign/2 (encode_payload_for_signing)
-spec capability_to_signable(capability()) -> binary().
capability_to_signable(#capability{} = Cap) ->
    %% Create the same payload format as reckon_gater_capability:encode_payload_for_signing/1
    ...
    %% Sort keys for deterministic serialization (must match reckon_gater_capability)
