%% @doc Unit tests for reckon_db_hash_nif module.
%%
...
-module(reckon_db_hash_nif_tests).
...
erlang_xxhash64_consistent_test() ->
    Data = small_data(),
    Hash1 = reckon_db_hash_nif:erlang_xxhash64(Data),
    Hash2 = reckon_db_hash_nif:erlang_xxhash64(Data),
    ?assertEqual(Hash1, Hash2).

erlang_xxhash64_different_data_test() ->
    Hash1 = reckon_db_hash_nif:erlang_xxhash64(<<"hello">>),
    Hash2 = reckon_db_hash_nif:erlang_xxhash64(<<"world">>),
    ?assertNotEqual(Hash1, Hash2).

erlang_xxhash64_with_seed_test() ->
    Data = small_data(),
    Hash1 = reckon_db_hash_nif:erlang_xxhash64(Data, 0),
    Hash2 = reckon_db_hash_nif:erlang_xxhash64(Data, 42),
    ?assertNotEqual(Hash1, Hash2).

erlang_xxhash3_consistent_test() ->
    Data = medium_data(),
    Hash1 = reckon_db_hash_nif:erlang_xxhash3(Data),
    Hash2 = reckon_db_hash_nif:erlang_xxhash3(Data),
    ?assertEqual(Hash1, Hash2).

erlang_partition_hash_range_test() ->
    Data = small_data(),
    Partitions = 16,
    Partition = reckon_db_hash_nif:erlang_partition_hash(Data, Partitions),
    ?assert(Partition >= 0),
...
erlang_partition_hash_consistent_test() ->
    Data = medium_data(),
    Partitions = 8,
    P1 = reckon_db_hash_nif:erlang_partition_hash(Data, Partitions),
    P2 = reckon_db_hash_nif:erlang_partition_hash(Data, Partitions),
    ?assertEqual(P1, P2).
...
erlang_stream_partition_test() ->
    ...
    StreamId = <<"stream-123">>,
    Partitions = 16,
    P = reckon_db_hash_nif:erlang_stream_partition(StoreId, StreamId, Partitions),
    ?assert(P >= 0),
    ?assert(P < Partitions).
...
erlang_partition_hash_batch_test() ->
    Items = [<<"a">>, <<"b">>, <<"c">>, <<"d">>],
    Partitions = 4,
    Results = reckon_db_hash_nif:erlang_partition_hash_batch(Items, Partitions),
...
erlang_fnv1a_consistent_test() ->
    Data = small_data(),
    Hash1 = reckon_db_hash_nif:erlang_fnv1a(Data),
    Hash2 = reckon_db_hash_nif:erlang_fnv1a(Data),
    ?assertEqual(Hash1, Hash2).

erlang_fnv1a_known_value_test() ->
    %% FNV-1a of empty string should be offset basis
    EmptyHash = reckon_db_hash_nif:erlang_fnv1a(<<>>),
    ?assertEqual(14695981039346656037, EmptyHash).

erlang_fast_phash_range_test() ->
    Data = medium_data(),
    Range = 100,
    Hash = reckon_db_hash_nif:erlang_fast_phash(Data, Range),
    ?assert(Hash >= 0),
    ?assert(Hash < Range).
...
api_xxhash64_test() ->
    Data = medium_data(),
    Hash = reckon_db_hash_nif:xxhash64(Data),
    ?assert(is_integer(Hash)),
    ?assert(Hash >= 0).

api_xxhash64_with_seed_test() ->
    Data = small_data(),
    Hash1 = reckon_db_hash_nif:xxhash64(Data, 0),
    Hash2 = reckon_db_hash_nif:xxhash64(Data, 12345),
    ?assert(is_integer(Hash1)),
    ?assert(is_integer(Hash2)).

api_xxhash3_test() ->
    Data = large_data(),
    Hash = reckon_db_hash_nif:xxhash3(Data),
    ?assert(is_integer(Hash)),
    ?assert(Hash >= 0).

api_partition_hash_test() ->
    Data = <<"stream-orders-12345">>,
    Partition = reckon_db_hash_nif:partition_hash(Data, 16),
    ?assert(Partition >= 0),
    ?assert(Partition < 16).
...
api_stream_partition_test() ->
    ...
    Partition = reckon_db_hash_nif:stream_partition(StoreId, StreamId, 8),
    ?assert(Partition >= 0),
    ?assert(Partition < 8).

api_partition_hash_batch_test() ->
    Items = [<<"item1">>, <<"item2">>, <<"item3">>],
    Results = reckon_db_hash_nif:partition_hash_batch(Items, 10),
...
api_fnv1a_test() ->
    Hash = reckon_db_hash_nif:fnv1a(<<"test">>),
    ?assert(is_integer(Hash)),
    ?assert(Hash >= 0).

api_fast_phash_test() ->
    Hash = reckon_db_hash_nif:fast_phash(<<"data">>, 1000),
    ?assert(Hash >= 0),
    ?assert(Hash < 1000).
...
implementation_returns_atom_test() ->
    Impl = reckon_db_hash_nif:implementation(),
    ?assert(Impl =:= nif orelse Impl =:= erlang).

is_nif_loaded_returns_boolean_test() ->
    Loaded = reckon_db_hash_nif:is_nif_loaded(),
...
nif_tests_when_available_test_() ->
    case reckon_db_hash_nif:is_nif_loaded() of
        true ->
            [
...
nif_xxhash64_works() ->
    Data = medium_data(),
    Hash = reckon_db_hash_nif:nif_xxhash64(Data),
    ?assert(is_integer(Hash)),
    ?assert(Hash >= 0),
    %% Consistent
    ?assertEqual(Hash, reckon_db_hash_nif:nif_xxhash64(Data)).

nif_xxhash3_works() ->
    Data = large_data(),
    Hash = reckon_db_hash_nif:nif_xxhash3(Data),
    ?assert(is_integer(Hash)),
    ?assert(Hash >= 0).

nif_partition_hash_works() ->
    Data = <<"test-stream">>,
    P = reckon_db_hash_nif:nif_partition_hash(Data, 16),
    ?assert(P >= 0),
    ?assert(P < 16).
...
nif_stream_partition_works() ->
    StoreId = <<"store">>,
    StreamId = <<"stream">>,
    P = reckon_db_hash_nif:nif_stream_partition(StoreId, StreamId, 8),
    ?assert(P >= 0),
    ?assert(P < 8).

nif_batch_works() ->
    Items = [<<"a">>, <<"b">>, <<"c">>],
    Results = reckon_db_hash_nif:nif_partition_hash_batch(Items, 4),
    ?assertEqual(3, length(Results)).

nif_fnv1a_works() ->
    Hash = reckon_db_hash_nif:nif_fnv1a(<<"test">>),
    ?assert(is_integer(Hash)),
    ?assert(Hash >= 0).
...
distribution_test() ->
    ...
    Results = reckon_db_hash_nif:partition_hash_batch(Items, Partitions),
...
benchmark_hashing() ->
    ...
    {TimeNif, _} = timer:tc(fun() ->
        [reckon_db_hash_nif:nif_partition_hash(Data, 16) || _ <- lists:seq(1, Iterations)]
    end),
