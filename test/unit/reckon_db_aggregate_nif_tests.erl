%% @doc Unit tests for reckon_db_aggregate_nif module.
%%
...
-module(reckon_db_aggregate_nif_tests).
...
erlang_aggregate_simple_test() ->
    ...
    Result = reckon_db_aggregate_nif:erlang_aggregate_events(Events, #{}, true),
...
erlang_aggregate_with_initial_state_test() ->
    ...
    Result = reckon_db_aggregate_nif:erlang_aggregate_events(Events, InitialState, true),
    ?assertEqual(150, maps:get(balance, Result)).

erlang_aggregate_no_finalize_test() ->
    Events = [#{balance => {sum, 100}}],
    Result = reckon_db_aggregate_nif:erlang_aggregate_events(Events, #{}, false),
...
erlang_sum_field_test() ->
    Events = simple_events(),
    Sum = reckon_db_aggregate_nif:erlang_sum_field(Events, amount),
    ?assertEqual(450, Sum).

erlang_sum_field_tagged_test() ->
    Events = tagged_events(),
    Sum = reckon_db_aggregate_nif:erlang_sum_field(Events, balance),
    ?assertEqual(120, Sum).

erlang_count_where_test() ->
    Events = simple_events(),
    Count = reckon_db_aggregate_nif:erlang_count_where(Events, status, active),
    ?assertEqual(2, Count).

erlang_merge_tagged_batch_test() ->
    Pairs = [{a, {sum, 10}}, {b, {sum, 20}}, {c, value}],
    State = #{a => {sum, 5}},
    Result = reckon_db_aggregate_nif:erlang_merge_tagged_batch(Pairs, State),
...
erlang_finalize_test() ->
    ...
    Result = reckon_db_aggregate_nif:erlang_finalize(Tagged),
...
erlang_aggregation_stats_test() ->
    Events = simple_events(),
    Stats = reckon_db_aggregate_nif:erlang_aggregation_stats(Events),
...
erlang_empty_events_test() ->
    Result = reckon_db_aggregate_nif:erlang_aggregate_events([], #{}, true),
    ?assertEqual(#{}, Result).
...
api_aggregate_events_test() ->
    Events = tagged_events(),
    Result = reckon_db_aggregate_nif:aggregate_events(Events, #{}, true),
    ?assertEqual(120, maps:get(balance, Result)).

api_sum_field_test() ->
    Events = simple_events(),
    Sum = reckon_db_aggregate_nif:sum_field(Events, amount),
    %% NIF returns float, Erlang returns integer - compare numerically
    ?assertEqual(450, trunc(Sum)).

api_count_where_test() ->
    Events = simple_events(),
    Count = reckon_db_aggregate_nif:count_where(Events, status, active),
    ?assertEqual(2, Count).

api_merge_tagged_batch_test() ->
    Pairs = [{x, {sum, 100}}],
    State = #{x => {sum, 50}},
    Result = reckon_db_aggregate_nif:merge_tagged_batch(Pairs, State),
    ?assert(is_map(Result)).

api_finalize_test() ->
    Tagged = #{val => {sum, 42}},
    Result = reckon_db_aggregate_nif:finalize(Tagged),
    ?assertEqual(42, maps:get(val, Result)).

api_aggregation_stats_test() ->
    Events = [#{a => 1}, #{b => 2}],
    Stats = reckon_db_aggregate_nif:aggregation_stats(Events),
...
implementation_returns_atom_test() ->
    Impl = reckon_db_aggregate_nif:implementation(),
    ?assert(Impl =:= nif orelse Impl =:= erlang).

is_nif_loaded_returns_boolean_test() ->
    Loaded = reckon_db_aggregate_nif:is_nif_loaded(),
...
nif_tests_when_available_test_() ->
    case reckon_db_aggregate_nif:is_nif_loaded() of
        true ->
            [
...
nif_aggregate_events_works() ->
    Events = tagged_events(),
    Result = reckon_db_aggregate_nif:nif_aggregate_events(Events, #{}, true),
    ?assertEqual(120, maps:get(balance, Result)).

nif_sum_field_works() ->
    Events = simple_events(),
    Sum = reckon_db_aggregate_nif:nif_sum_field(Events, amount),
    ?assert(Sum >= 449 andalso Sum =< 451). %% Allow small float differences

nif_count_where_works() ->
    Events = simple_events(),
    Count = reckon_db_aggregate_nif:nif_count_where(Events, status, active),
    ?assertEqual(2, Count).

nif_merge_tagged_batch_works() ->
    Pairs = [{a, {sum, 10}}],
    State = #{a => {sum, 5}},
    Result = reckon_db_aggregate_nif:nif_merge_tagged_batch(Pairs, State),
    ?assert(is_map(Result)).

nif_finalize_works() ->
    Tagged = #{val => {sum, 42}},
    Result = reckon_db_aggregate_nif:nif_finalize(Tagged),
    ?assertEqual(42, maps:get(val, Result)).

nif_aggregation_stats_works() ->
    Events = simple_events(),
    Stats = reckon_db_aggregate_nif:nif_aggregation_stats(Events),
    ?assertEqual(3, maps:get(total_events, Stats)).
...
equivalence_aggregate_events() ->
    Events = tagged_events(),
    ErlResult = reckon_db_aggregate_nif:erlang_aggregate_events(Events, #{}, true),
    NifResult = reckon_db_aggregate_nif:nif_aggregate_events(Events, #{}, true),
...
equivalence_finalize() ->
    ...
    ErlResult = reckon_db_aggregate_nif:erlang_finalize(Tagged),
    NifResult = reckon_db_aggregate_nif:nif_finalize(Tagged),
...
benchmark_aggregation() ->
    ...
    {TimeErl, _} = timer:tc(fun() ->
        [reckon_db_aggregate_nif:erlang_aggregate_events(Events, #{}, true)
         || _ <- lists:seq(1, Iterations)]
    end),

    %% Benchmark NIF path
    {TimeNif, _} = timer:tc(fun() ->
        [reckon_db_aggregate_nif:nif_aggregate_events(Events, #{}, true)
         || _ <- lists:seq(1, Iterations)]
    end),
