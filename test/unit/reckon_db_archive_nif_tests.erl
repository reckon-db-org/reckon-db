%% @doc Unit tests for reckon_db_archive_nif module.
%%
...
-module(reckon_db_archive_nif_tests).
...
erlang_zlib_roundtrip_test() ->
    Data = small_data(),
    {ok, Compressed} = reckon_db_archive_nif:erlang_compress_zlib(Data, 6),
    {ok, Decompressed} = reckon_db_archive_nif:erlang_decompress_zlib(Compressed),
...
erlang_zlib_levels_test() ->
    ...
    lists:foreach(fun(Level) ->
        {ok, Compressed} = reckon_db_archive_nif:erlang_compress_zlib(Data, Level),
        {ok, Decompressed} = reckon_db_archive_nif:erlang_decompress_zlib(Compressed),
        ?assertEqual(Data, Decompressed)
    end, [0, 1, 5, 9]).
...
erlang_lz4_fallback_roundtrip_test() ->
    ...
    Data = small_data(),
    {ok, Compressed} = reckon_db_archive_nif:erlang_compress_lz4(Data),
    {ok, Decompressed} = reckon_db_archive_nif:erlang_decompress_lz4(Compressed),
    ?assertEqual(Data, Decompressed).

erlang_zstd_fallback_roundtrip_test() ->
    %% Zstd fallback uses zlib internally
    Data = small_data(),
    {ok, Compressed} = reckon_db_archive_nif:erlang_compress_zstd(Data, 3),
    {ok, Decompressed} = reckon_db_archive_nif:erlang_decompress_zstd(Compressed),
    ?assertEqual(Data, Decompressed).
...
api_compress_lz4_test() ->
    ...
    {ok, Decompressed} = reckon_db_archive_nif:decompress(Compressed, lz4),
    ?assertEqual(Data, Decompressed).

api_compress_zstd_test() ->
    Data = medium_data(),
    {ok, Compressed} = reckon_db_archive_nif:compress(Data, zstd),
    ?assert(is_binary(Compressed)),
    {ok, Decompressed} = reckon_db_archive_nif:decompress(Compressed, zstd),
    ?assertEqual(Data, Decompressed).

api_compress_zlib_test() ->
    Data = medium_data(),
    {ok, Compressed} = reckon_db_archive_nif:compress(Data, zlib),
    ?assert(is_binary(Compressed)),
    {ok, Decompressed} = reckon_db_archive_nif:decompress(Compressed, zlib),
    ?assertEqual(Data, Decompressed).

api_compress_with_level_test() ->
    Data = compressible_data(),
    %% Zstd with level
    {ok, Compressed1} = reckon_db_archive_nif:compress(Data, zstd, 1),
    {ok, Compressed9} = reckon_db_archive_nif:compress(Data, zstd, 9),
...
api_compression_stats_test() ->
    Data = compressible_data(),
    {ok, Stats} = reckon_db_archive_nif:compression_stats(Data, zlib),
...
implementation_returns_atom_test() ->
    Impl = reckon_db_archive_nif:implementation(),
    ?assert(Impl =:= nif orelse Impl =:= erlang).

is_nif_loaded_returns_boolean_test() ->
    Loaded = reckon_db_archive_nif:is_nif_loaded(),
...
nif_tests_when_available_test_() ->
    case reckon_db_archive_nif:is_nif_loaded() of
        true ->
            [
...
nif_lz4_roundtrip() ->
    Data = medium_data(),
    Compressed = reckon_db_archive_nif:nif_compress_lz4(Data),
    ?assert(is_binary(Compressed)),
    {ok, Decompressed} = reckon_db_archive_nif:nif_decompress_lz4(Compressed),
    ?assertEqual(Data, Decompressed).

nif_zstd_roundtrip() ->
    Data = medium_data(),
    Compressed = reckon_db_archive_nif:nif_compress_zstd(Data, 3),
    ?assert(is_binary(Compressed)),
    {ok, Decompressed} = reckon_db_archive_nif:nif_decompress_zstd(Compressed),
    ?assertEqual(Data, Decompressed).

nif_zlib_roundtrip() ->
    Data = medium_data(),
    Compressed = reckon_db_archive_nif:nif_compress_zlib(Data, 6),
    ?assert(is_binary(Compressed)),
    {ok, Decompressed} = reckon_db_archive_nif:nif_decompress_zlib(Compressed),
    ?assertEqual(Data, Decompressed).
...
nif_large_data() ->
    ...
    lists:foreach(fun(Algo) ->
        {ok, Compressed} = reckon_db_archive_nif:compress(Data, Algo),
        {ok, Decompressed} = reckon_db_archive_nif:decompress(Compressed, Algo),
        ?assertEqual(Data, Decompressed)
    end, [lz4, zstd, zlib]).

nif_compression_stats() ->
    Data = compressible_data(),
    {OrigSize, CompSize, Ratio} = reckon_db_archive_nif:nif_compression_stats(Data, lz4, 0),
...
equivalence_zlib() ->
    ...
    {ok, ErlCompressed} = reckon_db_archive_nif:erlang_compress_zlib(Data, 6),
    NifCompressed = reckon_db_archive_nif:nif_compress_zlib(Data, 6),

    %% Decompress each with the other
    {ok, ErlDecomp} = reckon_db_archive_nif:erlang_decompress_zlib(NifCompressed),
    {ok, NifDecomp} = reckon_db_archive_nif:nif_decompress_zlib(ErlCompressed),
...
benchmark_compression() ->
    ...
    {TimeErl, _} = timer:tc(fun() ->
        [reckon_db_archive_nif:erlang_compress_zlib(Data, 6)
         || _ <- lists:seq(1, Iterations)]
    end),

    %% Benchmark NIF zlib
    {TimeNif, _} = timer:tc(fun() ->
        [reckon_db_archive_nif:nif_compress_zlib(Data, 6)
         || _ <- lists:seq(1, Iterations)]
    end),
...
algorithm_comparison_test() ->
    ...
    {ok, LZ4Stats} = reckon_db_archive_nif:compression_stats(Data, lz4),
    {ok, ZstdStats} = reckon_db_archive_nif:compression_stats(Data, zstd),
    {ok, ZlibStats} = reckon_db_archive_nif:compression_stats(Data, zlib),
