%% @doc Unit tests for reckon_db_crypto_nif module.
%%
...
-module(reckon_db_crypto_nif_tests).
...
erlang_verify_ed25519_valid_test() ->
    ...
    ?assert(reckon_db_crypto_nif:erlang_verify_ed25519(Message, Signature, PubKey)).

erlang_verify_ed25519_invalid_sig_test() ->
    {PubKey, _PrivKey} = generate_test_keypair(),
    Message = <<"Hello, World!">>,
    FakeSig = crypto:strong_rand_bytes(64),
    ?assertNot(reckon_db_crypto_nif:erlang_verify_ed25519(Message, FakeSig, PubKey)).
...
erlang_verify_ed25519_wrong_message_test() ->
    ...
    ?assertNot(reckon_db_crypto_nif:erlang_verify_ed25519(Message2, Signature, PubKey)).
...
erlang_verify_ed25519_wrong_key_test() ->
    ...
    ?assertNot(reckon_db_crypto_nif:erlang_verify_ed25519(Message, Signature, PubKey2)).

erlang_hash_sha256_test() ->
    Data = <<"test data">>,
    Expected = crypto:hash(sha256, Data),
    ?assertEqual(Expected, reckon_db_crypto_nif:erlang_hash_sha256(Data)).
...
erlang_hash_sha256_base64_test() ->
    ...
    ?assertEqual(Expected, reckon_db_crypto_nif:erlang_hash_sha256_base64(Data)).

erlang_base64_encode_urlsafe_test() ->
    Data = <<1, 2, 3, 4, 5>>,
    Expected = base64:encode(Data, #{mode => urlsafe, padding => false}),
    ?assertEqual(Expected, reckon_db_crypto_nif:erlang_base64_encode_urlsafe(Data)).

erlang_base64_decode_urlsafe_test() ->
    Original = <<1, 2, 3, 4, 5>>,
    Encoded = base64:encode(Original, #{mode => urlsafe, padding => false}),
    ?assertEqual({ok, Original}, reckon_db_crypto_nif:erlang_base64_decode_urlsafe(Encoded)).

erlang_base64_decode_invalid_test() ->
    Invalid = <<"!!!not valid base64!!!">>,
    ?assertEqual({error, invalid_base64}, reckon_db_crypto_nif:erlang_base64_decode_urlsafe(Invalid)).

erlang_secure_compare_equal_test() ->
    A = <<"secret_token_123">>,
    B = <<"secret_token_123">>,
    ?assert(reckon_db_crypto_nif:erlang_secure_compare(A, B)).

erlang_secure_compare_not_equal_test() ->
    A = <<"secret_token_123">>,
    B = <<"secret_token_456">>,
    ?assertNot(reckon_db_crypto_nif:erlang_secure_compare(A, B)).
...
erlang_secure_compare_empty_test() ->
    ?assert(reckon_db_crypto_nif:erlang_secure_compare(<<>>, <<>>)).
...
api_verify_ed25519_test() ->
    ...
    Signature = sign_message(Message, PrivKey),
    ?assert(reckon_db_crypto_nif:verify_ed25519(Message, Signature, PubKey)).

api_hash_sha256_test() ->
    Data = <<"API test data">>,
    Expected = crypto:hash(sha256, Data),
    ?assertEqual(Expected, reckon_db_crypto_nif:hash_sha256(Data)).

api_hash_sha256_base64_test() ->
    Data = <<"API test data">>,
    Result = reckon_db_crypto_nif:hash_sha256_base64(Data),
    ?assert(is_binary(Result)),
    %% Should be valid base64 and decode to 32 bytes (SHA256)
    {ok, Decoded} = reckon_db_crypto_nif:base64_decode_urlsafe(Result),
    ?assertEqual(32, byte_size(Decoded)).

api_base64_roundtrip_test() ->
    Original = crypto:strong_rand_bytes(100),
    Encoded = reckon_db_crypto_nif:base64_encode_urlsafe(Original),
    {ok, Decoded} = reckon_db_crypto_nif:base64_decode_urlsafe(Encoded),
    ?assertEqual(Original, Decoded).

api_secure_compare_test() ->
    Token = crypto:strong_rand_bytes(32),
    ?assert(reckon_db_crypto_nif:secure_compare(Token, Token)),
    ?assertNot(reckon_db_crypto_nif:secure_compare(Token, crypto:strong_rand_bytes(32))).
...
implementation_returns_atom_test() ->
    Impl = reckon_db_crypto_nif:implementation(),
    ?assert(Impl =:= nif orelse Impl =:= erlang).

is_nif_loaded_returns_boolean_test() ->
    Loaded = reckon_db_crypto_nif:is_nif_loaded(),
...
nif_tests_when_available_test_() ->
    case reckon_db_crypto_nif:is_nif_loaded() of
        true ->
...
nif_verify_ed25519_works() ->
    ...
    Signature = sign_message(Message, PrivKey),
    ?assert(reckon_db_crypto_nif:nif_verify_ed25519(Message, Signature, PubKey)).

nif_hash_sha256_works() ->
    Data = <<"NIF test data">>,
    Expected = crypto:hash(sha256, Data),
    ?assertEqual(Expected, reckon_db_crypto_nif:nif_hash_sha256(Data)).

nif_hash_sha256_base64_works() ->
    Data = <<"NIF test data">>,
    Result = reckon_db_crypto_nif:nif_hash_sha256_base64(Data),
    ?assert(is_binary(Result)),
    ?assert(byte_size(Result) > 0).

nif_base64_roundtrip_works() ->
    Original = crypto:strong_rand_bytes(100),
    Encoded = reckon_db_crypto_nif:nif_base64_encode_urlsafe(Original),
    {ok, Decoded} = reckon_db_crypto_nif:nif_base64_decode_urlsafe(Encoded),
    ?assertEqual(Original, Decoded).

nif_secure_compare_works() ->
    Token = crypto:strong_rand_bytes(32),
    ?assert(reckon_db_crypto_nif:nif_secure_compare(Token, Token)),
    ?assertNot(reckon_db_crypto_nif:nif_secure_compare(Token, crypto:strong_rand_bytes(32))).
...
equivalence_verify_ed25519() ->
    ...
    ErlResult = reckon_db_crypto_nif:erlang_verify_ed25519(Message, Signature, PubKey),
    NifResult = reckon_db_crypto_nif:nif_verify_ed25519(Message, Signature, PubKey),
    ?assertEqual(ErlResult, NifResult).

equivalence_hash_sha256() ->
    Data = crypto:strong_rand_bytes(1000),
    ErlResult = reckon_db_crypto_nif:erlang_hash_sha256(Data),
    NifResult = reckon_db_crypto_nif:nif_hash_sha256(Data),
    ?assertEqual(ErlResult, NifResult).

equivalence_hash_sha256_base64() ->
    Data = crypto:strong_rand_bytes(1000),
    ErlResult = reckon_db_crypto_nif:erlang_hash_sha256_base64(Data),
    NifResult = reckon_db_crypto_nif:nif_hash_sha256_base64(Data),
    ?assertEqual(ErlResult, NifResult).

equivalence_base64_encode() ->
    Data = crypto:strong_rand_bytes(100),
    ErlResult = reckon_db_crypto_nif:erlang_base64_encode_urlsafe(Data),
    NifResult = reckon_db_crypto_nif:nif_base64_encode_urlsafe(Data),
    ?assertEqual(ErlResult, NifResult).
...
equivalence_secure_compare() ->
    ...
    ErlEq = reckon_db_crypto_nif:erlang_secure_compare(A, A),
    NifEq = reckon_db_crypto_nif:nif_secure_compare(A, A),
    ?assertEqual(ErlEq, NifEq),

    ErlNeq = reckon_db_crypto_nif:erlang_secure_compare(A, B),
    NifNeq = reckon_db_crypto_nif:nif_secure_compare(A, B),
...
benchmark_verify_ed25519() ->
    ...
    {TimeErl, _} = timer:tc(fun() ->
        [reckon_db_crypto_nif:erlang_verify_ed25519(Message, Signature, PubKey)
         || _ <- lists:seq(1, Iterations)]
    end),

    %% Benchmark NIF path
    {TimeNif, _} = timer:tc(fun() ->
        [reckon_db_crypto_nif:nif_verify_ed25519(Message, Signature, PubKey)
         || _ <- lists:seq(1, Iterations)]
    end),
