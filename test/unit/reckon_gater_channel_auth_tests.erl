...
-module(reckon_gater_channel_auth_tests).
...
setup() ->
    ...
    case pg:start(reckon_gater_channel_scope) of
        {ok, _} -> ok;
        {error, {already_started, _}} -> ok
    end,

    %% Start a test channel
    {ok, Pid} = reckon_gater_channel_server:start_link(test_channel_impl, #{
...
create_test_capability(Issuer, Resource, Action, TTL) ->
    ...
    reckon_gater_capability:encode(SignedCap, binary).
...
create_expired_capability(Issuer, Resource, Action) ->
    ...
    SignedCap = reckon_gater_capability:sign(ExpiredCap, reckon_gater_identity:private_key(Issuer)),
    reckon_gater_capability:encode(SignedCap, binary).
...
publish_with_valid_cap() ->
    ...
    Result = reckon_gater_channel_server:publish(
        test_auth_channel,
        <<"events.user.created">>,
        #{event => <<"user_created">>},
        Token
...
publish_with_invalid_cap() ->
    ...
    Grants = [reckon_gater_capability:grant(Resource, ?ACTION_CHANNEL_PUBLISH)],
    Audience = reckon_gater_identity:did(create_test_identity()),
...
subscribe_with_valid_cap() ->
    ...
    Result = reckon_gater_channel_server:subscribe(
        test_auth_channel,
        <<"events.user.created">>,
        self(),
        Token
    ...
    %% Cleanup - unsubscribe
    reckon_gater_channel_server:unsubscribe(test_auth_channel, <<"events.user.created">>, self()).
...
wildcard_resource_matches() ->
    ...
    Result1 = reckon_gater_channel_server:publish(
        test_auth_channel,
        <<"events.user.created">>,
        #{event => <<"test1">>},
        Token
    ...
    Result2 = reckon_gater_channel_server:publish(
        test_auth_channel,
        <<"metrics.cpu.usage">>,
        #{event => <<"test2">>},
        Token
