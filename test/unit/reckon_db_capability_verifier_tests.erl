%% @doc Unit tests for reckon_db_capability_verifier
%%
...
-module(reckon_db_capability_verifier_tests).
...
create_future_capability(Issuer, Audience, Grants) ->
    Cap = reckon_gater_capability:create(Issuer, Audience, Grants, #{ttl => 3600}),
...
verify_valid_token() ->
    ...
    Audience = reckon_gater_identity:did(create_test_identity()),

    Grants = [
        reckon_gater_capability:grant(<<"esdb://test/stream/*">>, ?ACTION_STREAM_READ)
    ],
    ...
    Result = reckon_db_capability_verifier:verify(Token),
    ?assertMatch({ok, #capability{}}, Result),
    {ok, VerifiedCap} = Result,
    ?assertEqual(reckon_gater_identity:did(Issuer), VerifiedCap#capability.iss),
...
verify_expired_token() ->
    ...
    Grants = [reckon_gater_capability:grant(<<"esdb://test/*">>, <<"*">>)],
...
verify_unsigned_token() ->
    ...
    %% reckon_gater_capability:encode/2 throws error for unsigned tokens,
...
verify_tampered_signature() ->
    ...
    Token = reckon_gater_capability:encode(TamperedCap, binary),
...
authorize_matching_grant() ->
    ...
    Result = reckon_db_capability_verifier:authorize(
        Token,
...
authorize_wildcard_grant() ->
    ...
    Result1 = reckon_db_capability_verifier:authorize(
        Token,
        <<"esdb://myapp/stream/orders">>,
        ?ACTION_STREAM_READ
    ),
    ...
    Result2 = reckon_db_capability_verifier:authorize(
        Token,
...
authorize_prefix_grant() ->
    ...
    Grants = [
        reckon_gater_capability:grant(<<"esdb://myapp/stream/orders-*">>, ?ACTION_STREAM_SUBSCRIBE)
    ],
...
authorize_wildcard_action() ->
    ...
    Grants = [
        reckon_gater_capability:grant(<<"esdb://myapp/*">>, ?ACTION_ADMIN_ALL)
    ],
...
check_permission_exact() ->
    ...
    ?assertEqual(ok, reckon_db_capability_verifier:check_permission(
        Cap,
...
check_permission_insufficient() ->
    ...
    Result = reckon_db_capability_verifier:check_permission(
        Cap,
...
token_cid_extraction() ->
    ...
    CID1 = reckon_db_capability_verifier:extract_token_cid(Cap1),
    CID2 = reckon_db_capability_verifier:extract_token_cid(Cap2),

    %% Different issuers should produce different CIDs
    ?assertNotEqual(CID1, CID2),

    %% Same token should produce same CID
    CID1Again = reckon_db_capability_verifier:extract_token_cid(Cap1),
...
match_resource_test_() ->
    [
        ?_assert(reckon_db_capability_verifier:match_resource(
            <<"esdb://test/stream/orders">>,
            <<"esdb://test/stream/orders">>
        )),
        ?_assertNot(reckon_db_capability_verifier:match_resource(
            <<"esdb://test/stream/orders">>,
...
match_action_test_() ->
    [
        ?_assert(reckon_db_capability_verifier:match_action(
            ?ACTION_STREAM_READ,
            ?ACTION_STREAM_READ
        )),
        ?_assertNot(reckon_db_capability_verifier:match_action(
            ?ACTION_STREAM_READ,
            ?ACTION_STREAM_APPEND
        )),
        ?_assert(reckon_db_capability_verifier:match_action(
