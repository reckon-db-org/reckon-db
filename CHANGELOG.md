...
- Disk sampling now tracks the **event-store disk**, not the container overlay
  root. The data dir is resolved lazily (retried each sample, since the store
  may not be configured yet at monitor init) and `df`-ed directly, so its true
  usage is reported and flagged `data_dir_mount` even when the container's
  busybox `df` mislabels the bind mount. Same-filesystem duplicates are
  collapsed. (Combines with 5.10.2's real-filesystem filter.)

## [5.10.2] - 2026-07-08

### Fixed

- Disk sampling now filters to **real filesystems**. Inside a container `df`
  lists ~20 pseudo mounts (tmpfs, proc, sysfs, cgroup, devtmpfs, shm, ...), which
  bloated `get_stats.disk` and fired a `[reckon_db, disk, sample]` event per
  pseudo mount every tick. Only devices that are a path (`/dev/...`) or the
  container root `overlay` are kept; zero-size mounts are dropped too.

## [5.10.1] - 2026-07-08

### Fixed

- **Disk sampling now works in containers.** `reckon_db_resource_monitor`
  sampled disk via `disksup`, which returns `[]` inside a container because it
  skips overlay/virtual filesystems (the only root a container overlay has) — so
  the `[reckon_db, disk, sample]` telemetry and `get_stats.disk` were empty on
  the exact deployment target. Disk is now read from `df -kP` (present on every
  Unix, works in containers and bare metal), with disksup kept as a fallback.
  CPU sampling was unaffected (cpu_sup reads /proc).
